...
import frontend.empleados.FrameRegistroUsuarioDeEmpleados;
...
public class GestionFrameEmpleadosShared {
	...
	FrameRegistroUsuarioDeEmpleados viewRegistro;
	...
	public void initController() {
		...
		view.getBtnAddEmp().addActionListener(
				e -> SwingUtil.exceptionWrapper(() -> seleccionarMenu(view.getBtnAddEmp().getActionCommand())));
		view.getBtnModEmp().addActionListener(
				e -> SwingUtil.exceptionWrapper(() -> seleccionarMenu(view.getBtnModEmp().getActionCommand())));
		view.getBtnRemEmp().addActionListener(
				e -> SwingUtil.exceptionWrapper(() -> seleccionarMenu(view.getBtnRemEmp().getActionCommand())));

		pnAdd.getBtAdd().addActionListener(e -> SwingUtil.exceptionWrapper(() -> addEmpleado()));
		pnAdd.getCbTipoEmpleado().addActionListener(e -> SwingUtil.exceptionWrapper(() -> establecerComboPuesto()));
	...
	private void addEmpleado() {
		if (camposCorrectosAdd()) {
			mostrarPnRegistroUsuario();
		}

	}

	private void mostrarPnRegistroUsuario() {
		viewRegistro = new FrameRegistroUsuarioDeEmpleados();
		initControllerRegistro();

		view.setVisible(false);
		viewRegistro.setVisible(true);
	}

	private void initControllerRegistro() {
		viewRegistro.getBtCancelar().addActionListener(e -> SwingUtil.exceptionWrapper(() -> cerrarVentanaRegistro()));
		viewRegistro.getBtAceptar().addActionListener(e -> SwingUtil.exceptionWrapper(() -> comprobarCamposRegistrarUsuario()));
	}


	private void comprobarCamposRegistrarUsuario() {
		...
		if (nombreUsuario.isBlank() || password.length == 0) {
		...
		else if (gesEmp.nombreUsuarioYaRegistrado(nombreUsuario)) {
			...
			registrarNuevoUsuarioEnBDD(nombreUsuario, passwordCifrada);
			
		}
	}

	private void registrarNuevoUsuarioEnBDD(String nombreUsuario, String passwordCifrada) {
		gesEmp.registrarEmpleadoComoUsuario(nombreUsuario, passwordCifrada);
		registrarNuevoEmpleado();
		JOptionPane.showMessageDialog(viewRegistro, "Se ha registrado correctamente el nuevo empleado",
	...
	private void registrarNuevoEmpleado() {
		...
		JOptionPane.showMessageDialog(pnAdd, "Se ha registrado al empleado correctamente",
				"Confirmación de Registro de Empleado", JOptionPane.INFORMATION_MESSAGE);
	...
	private boolean camposCorrectosAdd() {
		...
		if (pnAdd.getTxNombre().getText().isBlank() || pnAdd.getTxApellido().getText().isBlank()
			...
			JOptionPane.showMessageDialog(pnAdd, "Se deben rellenar todos los campos", "Error en Registro de Empleado",
					JOptionPane.ERROR_MESSAGE);
			return false;
		}
		...
		if (gesEmp.isDniYaRegistrado(pnAdd.getTxDNI().getText())) {
			JOptionPane.showMessageDialog(pnAdd, "Este DNI ya está registrado para otro empleado",
					"Error en Registro de Empleado", JOptionPane.ERROR_MESSAGE);
			return false;
		}
		return true;
	...
	private void modEmpleado() {
		if (camposCorrectosMod()) {
			if (!pnMod.getListEmpleados().isSelectionEmpty()) {
				...
				JOptionPane.showMessageDialog(pnMod, "Se ha modificado al empleado correctamente",
						"Confirmación de Modificación de Empleado", JOptionPane.INFORMATION_MESSAGE);
				inicializarPanelMod();
			} else {
				JOptionPane.showMessageDialog(pnMod, "Se debe seleccionar un empleado a modificar",
						"Error en Modificación de Empleado", JOptionPane.ERROR_MESSAGE);
			}
		}
	...
	private boolean camposCorrectosMod() {
		...
		if (pnMod.getTxNombre().getText().isBlank() || pnMod.getTxApellido().getText().isBlank()
			...
			JOptionPane.showMessageDialog(pnMod, "Se deben rellenar todos los campos",
					"Error en Modificación de Empleado", JOptionPane.ERROR_MESSAGE);
			return false;
		}
		...
		if (!pnMod.getListEmpleados().getSelectedValue().getPuesto().equals(PuestoEmpleado.JUGADOR)
			...
			JOptionPane.showMessageDialog(pnMod, "Solo los jugadores pueden ser menores de edad",
					"Error en Modificación de Empleado", JOptionPane.ERROR_MESSAGE);
			return false;
		}
		return true;
	...
	private void eliminarEmpleado() {
		...
		JOptionPane.showMessageDialog(pnRem, "Se ha eliminado al empleado correctamente",
				"Confirmación de Eliminación de Empleado", JOptionPane.INFORMATION_MESSAGE);
		inicializarPanelRem();
	}
